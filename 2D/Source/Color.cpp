...
{
	assert(blend_func);  //blend_func cannot be a nullptr
...
{
	uint8_t alpha = src.a;
	uint8_t inv_alpha = 255 - src.a; //inverse alpha

	color_t color;
	color.r = (src.r * alpha + dest.r * inv_alpha) >> 8;
	color.g = (src.g * alpha + dest.g * inv_alpha) >> 8;
	color.b = (src.b * alpha + dest.b * inv_alpha) >> 8;  //>> 8 is essentially dividing by 256 (puts colors back in range)
	color.a = src.a;
...
color_t AdditiveBlend(const color_t& src, const color_t& dest)
{
	...
	color.b = std::min(src.b + dest.b, 255);  //>> 8 is essentially dividing by 256 (puts colors back in range)
	color.a = src.a;
...
color_t MultiplyBlend(const color_t& src, const color_t& dest)
{
	...
	color.b = (src.b * dest.b) >> 8;  //>> 8 is essentially dividing by 256 (puts colors back in range)
	color.a = src.a;

	return color;
}

//multiply will be the same as additive, but you will multiply and then shift it back into range